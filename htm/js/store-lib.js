...
var sF$=(function(){
    ...
    if(aImgsShare)
      for(var i=0;i<aImgsShare.length;i++){
        ...
        if(aImgsShare[i].className=='EstImgShareFacebook'){
          ...
          aImgsShare[i].setAttribute(sSrc,FC$.PathImg +'iconprodgoogleplus.svg?cccfc=1');
          if(FC$.LazyLoad>0)aImgsShare[i].setAttribute("loading","lazy");
  ...
  function fnLoginUserName(NameUser,PicUser){
    ...
    if(NameUser==""){
      document.getElementById('fc-loginInfo').innerHTML = "<a href='"+ FCLib$.uk("url-register") +"?pp=3&passo=1&sit=1'><img src="+ FC$.PathImg +"iconuser.svg width=30 height=30"+ sLazy +"></a>";
      if(oImgGlobalSign){oImgGlobalSign.style.display="";}
    }
    else{
      NameUser=fnFirstName(NameUser);
      document.getElementById('fc-loginInfo').innerHTML = "<a href='#Logout' onclick=FCLib$.fnClientLogout('',sF$.fnCliLogout)><span class=HeaderSocialLoginLogout><img src="+ FC$.PathImg +"iconuser-off.svg width=30 height=30"+ sLazy +"></span></a>";
